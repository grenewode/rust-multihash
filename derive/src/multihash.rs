            ...
            impl multihash::MultihashDigest for Multihash {
                fn new(code: u64, input: &[u8]) -> Result<Self, multihash::Error> {
                    match code {
                        ...
                        0x38b64f => Ok(Self::Strobe256(multihash::Strobe256::digest(input))),
                ...
                fn wrap(code: u64, digest: &[u8]) -> Result<Self, multihash::Error> {
                    match code {
                        ...
                        0x38b64f => Ok(Self::Strobe256(multihash::Digest::wrap(digest)?)),
                        _ => Err(multihash::Error::UnsupportedCode(code)),
                    }
                }
                fn code(&self) -> u64 {
                ...
                fn size(&self) -> u8 {
                    match self {
                        Multihash::Identity256(mh) => multihash::Digest::size(mh),
                        Multihash::Strobe256(mh) => multihash::Digest::size(mh),
                    }
                }
                fn digest(&self) -> &[u8] {
                    match self {
                    ...
                    Self: Sized
                {
                    let code = unsigned_varint::io::read_u64(&mut r)?;
                    match code {
                        multihash::IDENTITY => Ok(Self::Identity256(multihash::Digest::from_reader(r)?)),
                        0x38b64f => Ok(Self::Strobe256(multihash::Digest::from_reader(r)?)),
                        _ => Err(multihash::Error::UnsupportedCode(code)),
                    }
                }
            }
    ...
    fn test_multihash_error_code_duplicates() {
        let input = quote! {
           ...
           pub enum Multihash {
               ...
               Identity256(multihash::Sha2Digest<U32>),
               #[mh(code = multihash::SHA2_256, hasher = multihash::Sha2_256)]
               Identity256(multihash::Sha2Digest<U32>),
            }
        };
